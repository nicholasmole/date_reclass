...
from json_datetime_util import jsondatetime
...
class DatetimeToJsonTest(unittest.TestCase):
  ...
  def test_error_convert_to_formatted_str(self):
    date = datetime(2020, 5, 17)
    # result = jsondatetime(date, 1).tostr()
    self.assertRaises(TypeError, jsondatetime(date, 1).tostr())

  def test_error_convert_to_formatted_datetime(self):
    date = '2020, 5, 17'
    self.assertRaises(TypeError, jsondatetime(date, 1).todatetime())